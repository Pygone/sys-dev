...
bool move_bing(const Position& pre_pos, const Position& pos)
{
	...
	if (is_our_side)
	{
		if (abs(pos.x - pre_pos.x) + abs(pos.y - pre_pos.y) != 1) return false; // 走一格
		if (pos.x < pre_pos.x) return false; // 只能往前走
		if (pos.x == pre_pos.x && pos.x <= 4) return false; // 过河前不能横着走
		if (pos.x < 0 || pos.x > 9 || pos.y < 0 || pos.y > 8) return false; // 越界
	}
	else
	{
		if (abs(pos.x - pre_pos.x) + abs(pos.y - pre_pos.y) != 1) return false; // 走一格
		if (pos.x > pre_pos.x) return false; // 只能往前走(考虑镜像故为>)
		if (pos.x == pre_pos.x && pos.x >= 5) return false; // 过河前不能横着走
		if (pos.x < 0 || pos.x > 9 || pos.y < 0 || pos.y > 8) return false;
	}
	movement(pre_pos, pos);
	return true;
